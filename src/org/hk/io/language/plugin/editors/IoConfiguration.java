...
public class IoConfiguration extends SourceViewerConfiguration {
    ...
    public String[] getConfiguredContentTypes(ISourceViewer sourceViewer) {
        return new String[] { IDocument.DEFAULT_CONTENT_TYPE, IoPartitionScanner.COMMENT, IoPartitionScanner.STRING, IoPartitionScanner.OPERATOR };
    }
    ...
    protected IoScanner getIoScanner() {
        if (ioScanner == null) {
            ...
            ioScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager.getColor(IIoColorConstants.KEYWORD))));
        }
        return ioScanner;
    }
    ...
    public IPresentationReconciler getPresentationReconciler(ISourceViewer sourceViewer) {
        ...
        NonRuleBasedDamagerRepairer ndr = new NonRuleBasedDamagerRepairer(new TextAttribute(colorManager.getColor(IIoColorConstants.COMMENT)));
        reconciler.setDamager(ndr, IoPartitionScanner.COMMENT);
        reconciler.setRepairer(ndr, IoPartitionScanner.COMMENT);

        ndr = new NonRuleBasedDamagerRepairer(new TextAttribute(colorManager.getColor(IIoColorConstants.OPERATOR)));
        reconciler.setDamager(ndr, IoPartitionScanner.OPERATOR);
        reconciler.setRepairer(ndr, IoPartitionScanner.OPERATOR);

        ndr = new NonRuleBasedDamagerRepairer(new TextAttribute(colorManager.getColor(IIoColorConstants.STRING)));
